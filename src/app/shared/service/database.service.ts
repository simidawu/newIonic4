...
export class DatabaseService {
  ...
  setUnreadToZeroByUserName(
    ...
    let sql;
    if (childType) {
      sql = `UPDATE MOA_LOCAL_MESSAGE SET UNREAD='N' WHERE OWNER='${owner}' AND FROM_USER_NAME='${username}' AND CHILD_TYPE='${childType}';`;
    } else {
  ...
  createMessageTable() {
    return this.database.executeSql(
      `CREATE TABLE IF NOT EXISTS MOA_LOCAL_MESSAGE
        ...
        TIME INTEGER, TYPE VARCHAR2(100),UNREAD VARCHAR2(100),EXTRA VARCHAR2(1000),CHILD_TYPE VARCHAR2(20),IMAGE_HEIGHT NUMBER,IMAGE_WIDTH NUMBER,DURATION NUMBER,VOUNREAD VARCHAR2(10),MSG_ID NUMBER);`,
        [],
    );
  }
  ...
  getAllUnreadCount(owner: string, toUsername: string) {
    return this.database.executeSql(
      `SELECT COUNT(*) COUNT FROM MOA_LOCAL_MESSAGE WHERE OWNER='${owner}' AND UNREAD='Y' AND TO_USER_NAME ='${toUsername}';`,
      [],
    );
  }
  ...
  async getMessageList(
    ...
    if (type) {
      if (child_type) {
        ...
        sql = `SELECT   A.*,
         (SELECT   COUNT ( * ) FROM   MOA_LOCAL_MESSAGE WHERE  OWNER='${loginUsername}' AND FROM_USER_NAME <> '${loginUsername}' AND  FROM_USER_NAME = A.FROM_USER_NAME AND UNREAD = 'Y') AS UNREAD_COUNT
         FROM   MOA_LOCAL_MESSAGE A,
         (  SELECT   FROM_USER_NAME, MAX (TIME) TIME
                FROM   MOA_LOCAL_MESSAGE
                WHERE  OWNER='${loginUsername}'
  ...
  findLastContent(msg: any[], loginUsername: string) {
    ...
    for (let i = 0; i < otherPeopleSendToMe.length; i++) {
      for (let j = 0; j < iSendToOtherPeople.length; j++) {
        if (
          otherPeopleSendToMe[i].fromUserName ===
          iSendToOtherPeople[j].toUserName
        ) {
          // 如果别人发给我的是最后一条信息
          if (otherPeopleSendToMe[i].time > iSendToOtherPeople[j].time) {
          } else {
            otherPeopleSendToMe[i].content = iSendToOtherPeople[j].content;
            otherPeopleSendToMe[i].contentType =
              iSendToOtherPeople[j].contentType;
    ...
    if (otherPeopleSendToMe.length > 0) {
      ...
      for (let i = 0; i < iSendToOtherPeople.length; i++) {
        ...
        for (let j = 0; j < otherPeopleSendToMe.length; j++) {
          // 我有发给别人，别人也有发给我
          if (
            iSendToOtherPeople[i].fromUserName ===
            otherPeopleSendToMe[j].toUserName &&
            iSendToOtherPeople[i].toUserName ===
            otherPeopleSendToMe[j].fromUserName
          ) {
            flag = false;
          }
        }
  ...
  addMessage(
    ...
    if (toUsername != fromUserName) {
      ...
      return this.database
        .executeSql(
          `INSERT INTO MOA_LOCAL_MESSAGE (TO_USER_NAME, FROM_USER_NAME,OWNER, CONTENT,CONTENT_TYPE,TIME,TYPE,UNREAD,EXTRA,CHILD_TYPE,IMAGE_HEIGHT,IMAGE_WIDTH,DURATION,VOUNREAD,MSG_ID)
        VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`,
          data,
        )
        .then(
  ...
  getAllMessages() {
    return this.database.executeSql('SELECT * FROM MOA_LOCAL_MESSAGE', []).then(
      data => {
        let msgs = [];
        if (data.rows.length > 0) {
          for (var i = 0; i < data.rows.length; i++) {
  ...
  dropTable() {
    return this.database
      .executeSql('DROP TABLE MOA_LOCAL_MESSAGE', [])
      .then(res => {
        return res;
      });
  }
  ...
  deleteMessagesByUser(
    ...
    return this.database.executeSql(
      `DELETE FROM MOA_LOCAL_MESSAGE WHERE OWNER='${owner}' AND  (FROM_USER_NAME='${fromUsername}' AND TO_USER_NAME='${toUserName}')
  ...
  createAvatarTable() {
    return this.database.executeSql(
      `CREATE TABLE IF NOT EXISTS MOA_LOCAL_AVATAR
  ...
  insertAvatarTable(
    ...
    if (userID) {
      ...
      return this.database.executeSql(
        'INSERT INTO MOA_LOCAL_AVATAR (USER_ID,USER_NAME,NICK_NAME,AVATAR) VALUES(?,?,?,?)',
        data,
      );
    } else {
      data = [username, nickname, avatar];
      return this.database.executeSql(
        'INSERT INTO MOA_LOCAL_AVATAR (USER_NAME,NICK_NAME,AVATAR) VALUES(?,?,?)',
        data,
      );
    }
  }
  ...
  getAvatarByUsername(username: string) {
    return this.database.executeSql(
      `SELECT * FROM MOA_LOCAL_AVATAR WHERE USER_NAME = '${username}' ;`,
      [],
    );
  }
  ...
  updateAvatarByUsername(username: string, avatar: string) {
    return this.database.executeSql(
      `UPDATE MOA_LOCAL_AVATAR SET AVATAR='${avatar}' WHERE USER_NAME='${username}'`,
      [],
    );
  }
  ...
  setvounreadByID(fromUsername: string, id: number) {
    return this.database.executeSql(
      `UPDATE MOA_LOCAL_MESSAGE SET VOUNREAD='N' WHERE FROM_USER_NAME='${fromUsername}' AND ID='${id}' `,
      [],
    );
  }
}
